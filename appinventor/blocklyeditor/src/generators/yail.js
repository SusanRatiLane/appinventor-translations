...
Blockly.Yail.getComponentLines = function(contextName, componentJson, parentName, componentMap,
  ...
  if (componentJson.$Type == 'Form') {
    code = Blockly.Yail.getFormPropertiesLines(contextName, componentJson, !forRepl, componentDb);
  } else if (componentJson.$Type == 'Task') {
    code = Blockly.Yail.getTaskPropertiesLines(contextName, componentJson, !forRepl, componentDb);
  } else {
 ...
 * @private
 */
Blockly.Yail.getComponentPropertiesLines = function(contextName, componentJson, parentName,
  ...
  code.push(Blockly.Yail.YAIL_ADD_COMPONENT +
    contextName + Blockly.Yail.YAIL_SPACER +
  ...
  code = code.concat(Blockly.Yail.getPropertySettersLines(contextName, componentJson, componentName, componentDb));
...
Blockly.Yail.getFormPropertiesLines = function(formName, componentJson, includeComments, componentDb) {
  ...
  var doAfterCreation = Blockly.Yail.YAIL_DO_AFTER_FORM_CREATION + formName + Blockly.Yail.YAIL_LINE_FEED
    + Blockly.Yail.YAIL_SPACER + Blockly.Yail.YAIL_REGISTER_FORM + Blockly.Yail.YAIL_QUOTE + formName
    + Blockly.Yail.YAIL_CLOSE_BLOCK;
  var yailForComponentProperties = Blockly.Yail.getPropertySettersLines(formName, componentJson, formName, componentDb);
 ...
 */
Blockly.Yail.getPropertySettersLines = function(contextName, componentJson, componentName, componentDb) {
  var code = [];
  for (var prop in componentJson) {
    if (prop.charAt(0) != "$" && prop != "Uuid" && prop != "TutorialURL") {
      code.push(Blockly.Yail.getPropertySetterString(contextName, componentName, componentJson.$Type, prop,
 ...
 * @private
 */
Blockly.Yail.getPropertySetterString = function(contextName, componentName, componentType, propertyName,
  ...
  var value = Blockly.Yail.getPropertyValueString(contextName, propertyValue, propType);
 ...
 * @private
 */
Blockly.Yail.getPropertyValueString = function(contextName, propertyValue, propertyType) {
  if (propertyType == "'number") {
    ...
    if (propertyValue == "") {
      ...
      return Blockly.Yail.YAIL_GET_COMPONENT + contextName + Blockly.Yail.YAIL_SPACER + propertyValue + ")";
    }
  }
 ...
 * @private
 */
Blockly.Yail.getComponentRenameString = function(contextName, oldName, newName) {
  return Blockly.Yail.YAIL_RENAME_COMPONENT + contextName
